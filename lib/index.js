...
function Server(tlsConfig, requestListener) {
  ...
  // access a specific subserver, you'll need to use ._{http,plainHttp2,https}Server
  ...
  this._plainHttp2Server = http2.createServer({}, boundListener);
  if (typeof tlsConfig === 'object') {
    this._httpsServer = http2.createSecureServer(
      Object.assign({ allowHTTP1: true }, tlsConfig),
      boundListener
    );
  } else {
    // Fake server that rejects all connections:
    this._httpsServer = new EventEmitter();
    this._httpsServer.on('connection', (socket) => socket.destroy());
  }

  const subServers = [this._httpServer, this._plainHttp2Server, this._httpsServer];
...
const http2Listener = function (socket, pastData) {
  const h2Server = this._plainHttp2Server;
